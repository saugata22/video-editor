...
// Calculate Mandelbrot set with smooth iteration count
...
// Calculate orbit trap (distance to origin during iteration)
vec2 mandelbrotOrbitTrap(vec2 c, int maxIter) {
  vec2 z = vec2(0.0);
...
void main() {
  ...
  if (uColorMode < 0.5) {
    ...
    float iter = mandelbrotSmooth(c, maxIter);
    ...
    vec2 videoUv = vec2(
      fract(bands * 2.0 + c.x * 0.3),
      fract(bands * 3.0 + c.y * 0.3)
    );
    ...
    vec2 trapPoint = mandelbrotOrbitTrap(c, maxIter);
